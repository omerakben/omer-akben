...
async function ensureIndex(name: string, command: unknown[], existsCommand: unknown[]) {
  const redis = getRedisClient();
  if (typeof redis.call !== "function") {
    throw new Error("[setup-redis-indexes] Redis client does not support raw call interface");
  }

  try {
    await redis.call(...existsCommand);
    console.info(`[setup-redis-indexes] Index '${name}' already exists, skipping`);
  } catch (error) {
    if (error instanceof Error && error.message.includes("Unknown Index")) {
      console.info(`[setup-redis-indexes] Creating index '${name}'`);
      await redis.call(...command);
      console.info(`[setup-redis-indexes] Index '${name}' created`);
    } else {
      console.error(`[setup-redis-indexes] Failed to verify index '${name}'`, error);
      throw error;
    }
  }
}
