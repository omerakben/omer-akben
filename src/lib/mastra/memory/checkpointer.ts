...
class RedisSaverAdapter {
  ...
  json = {
    get: (...args: Parameters<RedisStackClient["json"]["get"]>) =>
      this.client.json.get(...args),
    set: (...args: Parameters<RedisStackClient["json"]["set"]>) =>
      this.client.json.set(...args),
  };
  ...
  async zAdd(
    ...
    const rest = entries
      .slice(1)
      .map(({ score, value }) => ({ score, member: value }));
    return this.client.zadd(key, first, ...rest);
  }

  del(...keys: Array<string | string[]>) {
    const flattened = keys.flatMap((item) =>
