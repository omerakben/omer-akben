...
test.describe("Agentic Sidebar - Keyboard Shortcuts", () => {
  ...
  test("should focus input when sidebar opens via keyboard shortcut", async ({ page }) => {
...
test.describe("Agentic Sidebar - Quick Actions", () => {
  ...
  test("should display quick action buttons", async ({ page }) => {
    ...
    const quickActions = page.locator('[class*="quick-action"], button[class*="rounded-lg"]');
  ...
  test("should display suggested questions", async ({ page }) => {
    ...
    const suggestions = page.locator('button:has-text("What problems"), button:has-text("Show me")');
  ...
  test("should send message when clicking suggested question", async ({ page }) => {
    // Click a suggested question
    const suggestionButton = page.locator('button:has-text("What problems")').first();
    await suggestionButton.click();

    // Wait for message to appear in chat
    await expect(page.locator('text=What problems')).toBeVisible({ timeout: 3000 });

    // Wait for AI response
    await expect(page.locator('[class*="assistant"]').filter({ hasText: /.+/ })).toBeVisible({ timeout: 15000 });
  });
});
...
test.describe("Agentic Sidebar - Message Interactions", () => {
  ...
  test("should send message and receive response", async ({ page }) => {
    ...
    // Submit
    const sendButton = page.locator('form#chat-sidebar-form button[type="submit"]');
  ...
  test("should render markdown in responses", async ({ page }) => {
    ...
    await expect(page.locator('ul li, ol li').first()).toBeVisible({ timeout: 15000 });
  });

  test("should display follow-up questions after assistant response", async ({ page }) => {
    // Send message
    const input = page.locator('input#chat-sidebar-input');
...
test.describe("Agentic Sidebar - Thread Persistence", () => {
  ...
  test("should clear thread after 24 hours (manual simulation)", async ({ page }) => {
...
test.describe("Agentic Sidebar - Navigation Links", () => {
  ...
  test("should display navigation link buttons in responses", async ({ page }) => {
    ...
    const navButtons = page.locator('button[class*="rounded-full"]').filter({ hasText: /View|See|Explore/ });
  ...
  test("should navigate internally when clicking internal link", async ({ page }) => {
    // Send message
    const input = page.locator('input#chat-sidebar-input');
    ...
    const projectsLink = page.locator('button:has-text("Projects"), button:has-text("View Projects")').first();
...
test.describe("Agentic Sidebar - Error Handling", () => {
  ...
  test("should handle network errors gracefully", async ({ page }) => {
    ...
    const hasRetry = await page.locator('button:has-text("Retry")').isVisible({ timeout: 1000 }).catch(() => false);
