...
test.describe("WIP Gate - Modal and Banner Flow", () => {
  ...
  test("should show WIP modal on first homepage visit", async ({ page }) => {
    ...
    const modal = page.locator('[role="dialog"]').filter({ hasText: /Welcome! Site Under Active Development/i });
    await expect(modal).toBeVisible();

    // Verify modal content
    await expect(modal).toContainText(/Thanks for visiting!/i);
    await expect(modal).toContainText(/This portfolio is actively being built/i);
    await expect(modal).toContainText(/Some features are still works-in-progress/i);
    ...
    const gotItButton = modal.locator('button', { hasText: /got it, let me explore/i });
  ...
  test("should show banner after modal dismissal", async ({ page }) => {
    ...
    const banner = page.locator('[role="banner"]').filter({ hasText: /Site under active development/i });
  ...
  test("should dismiss banner and persist to localStorage", async ({ page }) => {
    ...
    const dismissButton = banner.locator('button[aria-label*="Dismiss banner" i]');
  ...
  test("should navigate to status page from banner link", async ({ page }) => {
  ...
  test("should maintain dismissal state across page reloads", async ({ page }) => {
...
test.describe("WIP Gate - Status Page", () => {
  test("should display unified status page with development info", async ({ page }) => {
  ...
  test("should be accessible from banner View status link", async ({ page }) => {
